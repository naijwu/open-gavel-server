...
router.post("/secretariat/register", async (req, res) => {
	...
	if (emailExists)
		return res
			.status(400)
			.json({ message: "Account with the email already exists" });

	// check for conference already existing
	const conferenceExists = await Secretariat.findOne({
		conference: req.body.conference,
	});
	if (conferenceExists)
		return res
			.status(400)
			.json({ message: "Conference is already registered" });
...
router.post("/staff/login", async (req, res) => {
	...
	const staffInConference = await Staff.find({
		conference_id: req.body.conference.toLowerCase(),
	});

	// check if username exists
	const staffInDB = staffInConference.find(
